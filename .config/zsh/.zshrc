...
zstyle ':vcs_info:git:*' formats       "%{$fg[blue]%}(%{$fg[black]%}%b%{$fg[blue]%}:%r%{$fg[yellow]%}%m%u%{$fg[magenta]%}%c%{$fg[blue]%})"
...
+vi-git-incoming-commits() {
    ...
    incoming_commits=$(git rev-list HEAD..origin/$(git rev-parse --abbrev-ref HEAD) --count)
