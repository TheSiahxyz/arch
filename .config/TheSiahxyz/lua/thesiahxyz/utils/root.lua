...
function M.detectors.lsp(buf)
  ...
  for _, client in pairs(thesiahxyz.lsp.get_clients({ bufnr = buf })) do
...
function M.info()
  ...
  lines[#lines + 1] = "```"
  thesiahxyz.info(lines, { title = "thesiahxyz Roots" })
...
function M.setup()
  vim.api.nvim_create_user_command("LazyRoot", function()
    thesiahxyz.root.info()
  end, { desc = "thesiahxyz roots for the current buffer" })

  vim.api.nvim_create_autocmd({ "LspAttach", "BufWritePost", "DirChanged" }, {
    group = vim.api.nvim_create_augroup("thesiahxyz_root_cache", { clear = true }),
...
function M.get(opts)
  ...
  return thesiahxyz.is_win() and ret:gsub("/", "\\") or ret
end
